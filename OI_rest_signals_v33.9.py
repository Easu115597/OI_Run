        ...
        if not os.path.exists(SUMMARY_CSV):
            with open(SUMMARY_CSV, 'w', newline='', encoding='utf-8') as f:
                ...
                writer.writerow([
                    ...
                    'Max_SmBase', 'Max_SmPhys', # ✨ 新增：摘要統計平滑版最高分
...
輸出JSON: {{"adjustment": -6~+3, "reason": "30字內理由"}}"""
    ...
    def log_to_csv(self, symbol, d, ai, v):
        ...
        try:
            ...
            row = [
                ...
                d.get('entry_stage', 'NONE'),           # 19
                d.get('decision', 'NO_GO'),             # 20
                v.get('structure', 'CHOP'),             # 21
                d.get('state_count', 0),                # 22
                str(ai.get('reason', 'N/A')).replace(',', ';'), # 23
                v.get('direction', 0),                  # 24
                'HOLD',                                 # 25
                d.get('reason_metadata', ''),           # 26 (上帝視角字串)
                1 if d.get('side') == 'LONG' else -1,   # 27
                'STAY'                                  # 28
            ]
    ...
    async def send_tg_alert(self, symbol, ai, d, v):
        ...
        if stage == "STAGE_1_RECON":
            ...
            stage_text = f"⚠️【異動觀察: {stage}】"

        # 如果分數沒到但被觸發，稍微標記一下
        if d.get('final_score', 0) < 75 and stage == "NONE": 
            stage_text = "⚠️【低分異動觀察】"
        if d['final_score'] < 75 and stage == "NONE": stage_text = "⚠️【異動觀察】"
